...
export async function seedDemoData() {
  try {
    ...
    if (Object.keys(users).length === 0) {
      ...
      const testUsers: Array<{user: User, password: string}> = [
        {
          user: {
            id: generateId(),
            firstName: 'Эльнур',
      ...
      for (const { user, password } of testUsers) {
        await FirebaseService.users.create(user.id, user)
        await FirebaseService.passwords.set(user.id, password)
      }

      console.log('✅ Demo admin created: FIN: ADMIN01, Password: admin123')
      console.log('✅ Demo users created: AA12345, BB67890, CC11223 (Password: 123456)')
    }

    const orders = await FirebaseService.orders.getAll()
    
    if (Object.keys(orders).length === 0 && Object.keys(users).length > 0) {
      const nonAdminUsers = Object.values(users).filter((u: User) => !u.isAdmin)
      
      if (nonAdminUsers.length > 0) {
        const userId = nonAdminUsers[0].id
        const now = Date.now()
